...
CSR_PostProcessingEffect::~CSR_PostProcessingEffect()
{
    ...
    if (m_pStaticBuffer)
        csrStaticBufferRelease(m_pStaticBuffer);
}
//---------------------------------------------------------------------------
void CSR_PostProcessingEffect::OnLinkStaticSurface(const CSR_Shader* pShader, const void* pParent)
{
    ...
    pEffect->m_pStaticBuffer           = csrStaticBufferCreate(pShader, &buffer);
