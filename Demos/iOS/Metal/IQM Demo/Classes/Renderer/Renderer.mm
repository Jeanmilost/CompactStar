...
CSR_CallbackController::CSR_CallbackController(void* _Nonnull pOwner)
{
    ...
    m_SceneContext.m_fOnGetIQMIndex   = OnGetIQMIndex;
}
//----------------------------------------------------------------------------
CSR_CallbackController::~CSR_CallbackController()
{}
...
//----------------------------------------------------------------------------
void CSR_CallbackController::OnGetIQMIndex(const CSR_IQM* _Nullable pIQM,
                                                 size_t*  _Nullable pAnimSetIndex,
                                                 size_t*  _Nullable pFrameIndex)
{
    [(__bridge id)m_pOwner OnGetIQMIndex :pIQM :pAnimSetIndex :pFrameIndex];
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
- (void) OnGetIQMIndex :(const CSR_IQM* _Nullable)pIQM
                       :(size_t*        _Nullable)pAnimSetIndex
                       :(size_t*        _Nullable)pFrameIndex
{
    *pAnimSetIndex = 0;
    *pFrameIndex   = m_Frame % m_MaxAnimFrame;
}
//----------------------------------------------------------------------------
...
- (void) BuildModelMatrix
{
    ...
    csrMat4Multiply(&intermediateMatrix, &rotateYMatrix, &m_ModelMatrix);

    // place it in the world
    m_ModelMatrix.m_Table[3][0] =  0.0f;
    m_ModelMatrix.m_Table[3][1] = -0.55f;
    m_ModelMatrix.m_Table[3][2] = -2.0f;
}
//----------------------------------------------------------------------------
- (void) InitScene :(nonnull MTKView*)pView
{
    ...
    [CSR_ObjectiveCHelper ResourceToFileName :@"mrfixit" :@"iqm" :&pFileName];

    // create the model
    CSR_IQM* pIQM = csrIQMOpen(pFileName, &vf, &vc, &material, 0, 0, 0, 0, 0, 0);
 
    free(pFileName);

    [self CreateBufferFromIQM :pIQM];

    // add the mesh to the scene
    CSR_SceneItem* pSceneItem = csrSceneAddIQM(m_pScene, pIQM, 0, 0);

    // get the resource body texture path
    NSURL* pUrl = [[NSBundle mainBundle]URLForResource: @"Body" withExtension:@"tga"];

    // load the body model texture
    if (![self CreateTexture :&pIQM->m_pMesh[0].m_Skin.m_Texture :pUrl])
        @throw @"The body model texture could not be loaded";

    // get the resource head texture path
    pUrl = [[NSBundle mainBundle]URLForResource: @"Head" withExtension:@"tga"];

    // load the head model texture
    if (![self CreateTexture :&pIQM->m_pMesh[1].m_Skin.m_Texture :pUrl])
        @throw @"The head model texture could not be loaded";

    // build the model matrix
    [self BuildModelMatrix];
