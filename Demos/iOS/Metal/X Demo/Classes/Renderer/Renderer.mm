...
CSR_CallbackController::CSR_CallbackController(void* _Nonnull pOwner)
{
    ...
    m_SceneContext.m_fOnGetXIndex     = OnGetXIndex;
}
//----------------------------------------------------------------------------
CSR_CallbackController::~CSR_CallbackController()
{}
...
//----------------------------------------------------------------------------
void CSR_CallbackController::OnGetXIndex(const CSR_X*  _Nullable pX,
                                               size_t* _Nullable pAnimSetIndex,
                                               size_t* _Nullable pFrameIndex)
{
    [(__bridge id)m_pOwner OnGetXIndex :pX :pAnimSetIndex :pFrameIndex];
}
//----------------------------------------------------------------------------
...
- (void) InitScene :(nonnull MTKView*)pView
{
    ...
    [CSR_ObjectiveCHelper ResourceToFileName :@"tiny_4anim" :@"x" :&pFileName];

    // create the model
    CSR_X* pX = csrXOpen(pFileName, &vf, &vc, 0, 0, 0, 0, 0, 0, 0);
 
    free(pFileName);

    [self CreateBufferFromX :pX];

    // add the mesh to the scene
    CSR_SceneItem* pSceneItem = csrSceneAddX(m_pScene, pX, 0, 0);

    // get the resource texture path
    NSURL* pUrl = [[NSBundle mainBundle]URLForResource: @"Tiny_skin" withExtension:@"bmp"];

    // load the model texture
    if (![self CreateTexture :&pX->m_pMesh[1].m_Skin.m_Texture :pUrl])
    ...
    m_ModelMatrix.m_Table[3][1] = -20.0f;
