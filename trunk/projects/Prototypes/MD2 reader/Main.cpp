    ...
    m_PreviousTime(::GetTickCount())
{
    ...
    {
    */
        m_MD2.Load("E:\\Jeanmilost\\QREngine\\Projects\\3D models reader\\TestDLL\\Resources\\Models\\Female_animated_2.md2");
        m_MD2.LoadNormals("E:\\Jeanmilost\\QREngine\\Projects\\3D models reader\\TestDLL\\Resources\\Tables\\Normals.bin");

        m_pMD2Animation = new QR_MD2Animation(&m_MD2, true);
        m_pMD2Animation->Add("Walk", 0, 39);
        m_pMD2Animation->Set("Walk");

        /*REM
        QR_MD2::IAnimationInfo info;
        info.m_Name  = "WALK";
        info.m_Start = 0;
        info.m_End   = 39;

        m_MD2.SetAnimation(info);
        */
    /*REM
    }
    */
...
{
    const double elapsedTime = (::GetTickCount() - m_PreviousTime);
    m_PreviousTime = ::GetTickCount();

    glClearColor(1.0f, 0.0f, 0.0f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT);// | GL_DEPTH_BUFFER_BIT);

    //REM m_MD2.Animate(elapsedTime, 50);

    DrawObjects(elapsedTime);
    glFlush();
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::DrawObjects(const double elapsedTime)
{
/*REM
    //GoldCube
    ...
    */
    ...
    QR_Vertex::IEFormat format = QR_Vertex::IE_VF_Colors;
    ...
    */
    ...
    m_MD2.GetVertexBuffer(vbs, QR_MD2::IE_L_None, light, format, false);
    */
    QR_MD2::ILight light;
    ...
    QR_Vertices vertices;
    m_MD2.GetVertices(index, vertices);
    ...
    QR_Vertices nextVertices;
    m_MD2.GetVertices(index, nextVertices);

    QR_Vertices interpolated;
    QR_MD2::Interpolate(0.5f, vertices, nextVertices, interpolated);

    QR_STDTools::DelAndClear(vertices);
    QR_STDTools::DelAndClear(nextVertices);
    */
    ...
    QR_Vertices interpolated;
    m_pMD2Animation->GetVertices(elapsedTime, 15, interpolated);
...
//            ::OutputDebugStringW(::FloatToStr(pBuffer->m_Data[j]).c_str());
        ...
        glVertexPointer(3, GL_FLOAT, vertexSize * sizeof(M_Precision), &interpolated[i]->m_Buffer[0]);
        ...
        if (format & QR_Vertex::IE_VF_Normals)
        {
            ...
            glNormalPointer(GL_FLOAT,
                            ...
                            &interpolated[i]->m_Buffer[offset]);
        ...
        // draw MD2 mesh
        switch (interpolated[i]->m_Type)
        {
            case QR_Vertex::IE_VT_TriangleStrip:
                glDrawArrays(GL_TRIANGLE_STRIP, 0, interpolated[i]->m_Buffer.size() / vertexSize);
                break;

            case QR_Vertex::IE_VT_TriangleFan:
                glDrawArrays(GL_TRIANGLE_FAN, 0, interpolated[i]->m_Buffer.size() / vertexSize);
                break;
        }
    ...
    */
    ...
    m_MD2.Draw(QRAPI_VF_None, false);
