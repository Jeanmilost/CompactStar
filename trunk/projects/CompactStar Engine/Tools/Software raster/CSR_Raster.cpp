...
int CSR_Raster::raster1()
{
    worldToCamera = Matrix44f(0.707107f, -0.331295f, 0.624695f, 0.0f, 0.0f, 0.883452f, 0.468521f, 0.0f, -0.707107f, -0.331295f, 0.624695f, 0.0f, -1.63871f, -5.747777f, -40.400412f, 1.0f);
    ...
    for (std::size_t i = 0; i < 1732; ++i)
        vertices[i] = Vec3f(verticesRAW[i * 3], verticesRAW[(i * 3) + 1], verticesRAW[(i * 3) + 2]);

    Vec2f st[3056];

    for (std::size_t i = 0; i < 3056; ++i)
        st[i] = Vec2f(verticesRAW[i * 2], verticesRAW[(i * 2) + 1]);
    ...
    */
    ...
    for (std::size_t y = 0; y < imageHeight; ++y)
    {
        ...
        {
            pLine[x].rgbtRed   = frameBuffer[y * imageWidth + x][0];
            pLine[x].rgbtGreen = frameBuffer[y * imageWidth + x][1];
            pLine[x].rgbtBlue  = frameBuffer[y * imageWidth + x][2];
        }
    }
