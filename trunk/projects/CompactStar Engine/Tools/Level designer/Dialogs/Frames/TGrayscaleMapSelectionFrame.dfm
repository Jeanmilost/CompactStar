object GrayscaleMapSelectionFrame: TGrayscaleMapSelectionFrame
  Left = 0
  Top = 0
  Width = 442
  Height = 106
  TabOrder = 0
  object paBackground: TPanel
    AlignWithMargins = True
    Left = 3
    Top = 3
    Width = 436
    Height = 100
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 0
    object imBitmap: TImage
      Left = 0
      Top = 0
      Width = 100
      Height = 100
      Align = alLeft
      Center = True
      Picture.Data = {
        
        0049454E44AE426082}
      Proportional = True
      Stretch = True
      OnDblClick = btInfoClick
      ExplicitLeft = -3
    end
    object paLeft: TPanel
      Left = 100
      Top = 0
      Width = 336
      Height = 100
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 0
      object laBitmapFile: TLabel
        AlignWithMargins = True
        Left = 3
        Top = 63
        Width = 330
        Height = 13
        Align = alBottom
        Caption = 'Grayscale bitmap file name'
        ExplicitWidth = 128
      end
      object paBitmapFile: TPanel
        Left = 0
        Top = 79
        Width = 336
        Height = 21
        Align = alBottom
        BevelOuter = bvNone
        TabOrder = 0
        object btBitmapFileOpen: TButton
          Left = 301
          Top = 0
          Width = 35
          Height = 21
          Align = alRight
          Caption = '...'
          TabOrder = 1
          OnClick = btBitmapFileOpenClick
        end
        object edBitmapFile: TEdit
          AlignWithMargins = True
          Left = 3
          Top = 0
          Width = 295
          Height = 21
          Margins.Top = 0
          Margins.Bottom = 0
          Align = alClient
          ReadOnly = True
          TabOrder = 0
        end
      end
      object paButtons: TPanel
        Left = 0
        Top = 0
        Width = 336
        Height = 25
        Align = alTop
        BevelOuter = bvNone
        TabOrder = 1
        object btInfo: TSpeedButton
          Left = 311
          Top = 0
          Width = 25
          Height = 25
          Cursor = crHandPoint
          Align = alRight
          Flat = True
          Glyph.Data = {
            
            A1A1A3A3A3ACACACC6C6C6DADADADADADADADADADADADADADADA}
          OnClick = btInfoClick
          ExplicitLeft = 255
          ExplicitTop = -3
        end
        object btConfig: TSpeedButton
          Left = 286
          Top = 0
          Width = 25
          Height = 25
          Cursor = crHandPoint
          Align = alRight
          Flat = True
          Glyph.Data = {
            
            A2A29F9F9FD8D8D8DADADADADADADADADADADADADADADADADADA}
          OnClick = btConfigClick
          ExplicitLeft = 255
        end
      end
    end
  end
  object odOpen: TOpenPictureDialog
    Options = [ofPathMustExist, ofFileMustExist, ofEnableSizing]
    Left = 119
    Top = 2
  end
  object pmConfig: TPopupMenu
    Left = 151
    Top = 2
    object miCopyToLocalDir: TMenuItem
      AutoCheck = True
      Caption = 'Copy to local dir'
    end
    object miDeleteBitmap: TMenuItem
      Caption = 'Delete bitmap'
      OnClick = miDeleteBitmapClick
    end
  end
end
