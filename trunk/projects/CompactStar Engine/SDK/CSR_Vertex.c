                                                 ...
                                                 const CSR_VertexBuffer*    pVB)
{
    ...
    // no vertex buffer to set static?
    if (!pVB)
        return 0;

    // create a static vertex buffer
    pSVB = csrStaticVertexBufferCreate();

    // succeeded?
    if (!pSVB)
        return 0;

    // copy all the vertex buffer properties, except the vertex buffer itself
    pSVB->m_Format   = pVB->m_Format;
    pSVB->m_Culling  = pVB->m_Culling;
    pSVB->m_Material = pVB->m_Material;
    pSVB->m_Time     = pVB->m_Time;

    // allocate and assign a Vertex Buffer Objects to the currently selected OpenGL handle
    glGenBuffers(1, &pSVB->m_BufferID);

    // bind the newly created VBO as being the active buffer and storing vertex attributes
    glBindBuffer(GL_ARRAY_BUFFER, pSVB->m_BufferID);
...
CSR_StaticVertexBuffer* csrStaticVertexBufferCreate(void)
{
    ...
    CSR_StaticVertexBuffer* pSVB = (CSR_StaticVertexBuffer*)malloc(sizeof(CSR_StaticVertexBuffer));

    // succeeded?
    if (!pSVB)
        return 0;

    // initialize the static vertex buffer content
    csrStaticVertexBufferInit(pSVB);
...
{
    // no static vertex buffer to release?
    if (!pSVB)
        return;

    // free the static vertex buffer content
    if (pSVB->m_BufferID != M_CSR_Error_Code)
        glDeleteBuffers(1, &pSVB->m_BufferID);

    // free the static vertex buffer
    free(pSVB);
}
//---------------------------------------------------------------------------
void csrStaticVertexBufferInit(CSR_StaticVertexBuffer* pSVB)
{
    ...
    // initialize the vertex format content
    csrVertexFormatInit(&pSVB->m_Format);

    // initialize the vertex culling content
    csrVertexCullingInit(&pSVB->m_Culling);

    // initialize the vertex material content
    csrMaterialInit(&pSVB->m_Material);

    // initialize the vertex buffer content
    pSVB->m_BufferID = M_CSR_Error_Code;
    pSVB->m_Time     = 0.0;
}
//---------------------------------------------------------------------------
// Mesh functions
...
{
    size_t          i;
    CSR_StaticMesh* pStaticMesh;

    // no shader?
    if (!pShader)
        return 0;

    // no shader attributes?
    if (!pSA)
        return 0;

    // no mesh to set static?
    if (!pMesh)
        return 0;

    // for each mesh vertex buffer, a shader attribute should be provided
    if (pSA->m_Count != pMesh->m_Count)
        return 0;

    // create a static mesh
    pStaticMesh = csrStaticMeshCreate();

    // succeeded?
    if (!pStaticMesh)
        return 0;

    // allocate the memory for the static vertex buffers
    pStaticMesh->m_pVB   = (CSR_StaticVertexBuffer*)malloc(pMesh->m_Count * sizeof(CSR_StaticVertexBuffer));
    pStaticMesh->m_Count =  pMesh->m_Count;

    // iterate through vertex buffer to mov on the GPU
    for (i = 0; i < pMesh->m_Count; ++i)
    {
        // make the next vertex buffer static
        CSR_StaticVertexBuffer* pSVB = csrVertexBufferSetStatic(pShader,
                                                               &pSA->m_pAttribute[i],
                                                               &pMesh->m_pVB[i]);
        ...
        if (!pSVB)
        {
            csrStaticMeshRelease(pStaticMesh);
            return 0;
        }

        // copy the static vertex buffer content
        pStaticMesh->m_pVB[i] = *pSVB;

        // free the static vertex buffer. NOTE don't use the release function, because the allocated
        // resources must be preserved
        free(pSVB);
    }

    // copy the mesh time property
    pStaticMesh->m_Time = pMesh->m_Time;

    // do move the textures in the static mesh? (NOTE the textures may only be kept on one mesh.
    // NOTE Do that on the end to avoid to loose the textures in case the process failed)
    if (!keepTexturesOnSource)
    {
        // copy the textures
        pStaticMesh->m_Shader = pMesh->m_Shader;
...
CSR_StaticMesh* csrStaticMeshCreate(void)
{
    ...
    CSR_StaticMesh* pStaticMesh = (CSR_StaticMesh*)malloc(sizeof(CSR_StaticMesh));

    // succeeded?
    if (!pStaticMesh)
        return 0;

    // initialize the static mesh content
    csrStaticMeshInit(pStaticMesh);
...
void csrStaticMeshRelease(CSR_StaticMesh* pStaticMesh)
{
    ...
    // no static mesh to release?
    if (!pStaticMesh)
        return;

    // delete the texture
    if (pStaticMesh->m_Shader.m_TextureID != M_CSR_Error_Code)
        glDeleteTextures(1, &pStaticMesh->m_Shader.m_TextureID);

    // delete the bump map
    if (pStaticMesh->m_Shader.m_BumpMapID != M_CSR_Error_Code)
        glDeleteTextures(1, &pStaticMesh->m_Shader.m_BumpMapID);

    // free the mesh content
    if (pStaticMesh->m_pVB)
    {
        // free the mesh vertex buffer content
        for (i = 0; i < pStaticMesh->m_Count; ++i)
            if (pStaticMesh->m_pVB[i].m_BufferID != M_CSR_Error_Code)
                glDeleteBuffers(1, &pStaticMesh->m_pVB[i].m_BufferID);

        // free the mesh vertex buffer
        free(pStaticMesh->m_pVB);
    }
...
{
    // no static mesh to initialize?
    if (!pStaticMesh)
        return;

    // initialize the texture shader content
    csrTextureShaderInit(&pStaticMesh->m_Shader);

    // initialize the mesh content
    pStaticMesh->m_pVB   = 0;
    pStaticMesh->m_Count = 0;
    pStaticMesh->m_Time  = 0.0;
}
//---------------------------------------------------------------------------
